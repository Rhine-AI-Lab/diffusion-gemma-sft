...
# python parse_and_get_acc.py --directory /data/oss_bucket_0/espo/nebula_308x/sudoku_llfree_dream_batchll_tlcv2_mu8_cl256_gc4_lr1e-5_kl5e-4_psi1.0_mc2/gl128/eval_results
def aggregate_all_checkpoints(root_dir):
    ckpt_dirs = sorted(
        d for d in glob.glob(os.path.join(root_dir, "checkpoint-*"))
        if os.path.isdir(d)
    )
    ...
    def update_best(acc_map, ckpt_name, ckpt_dir):
        nonlocal best
        for setup, acc in acc_map.items():
    ...
    for ckpt_dir in ckpt_dirs:
        ...
        print(f"\n=== {ckpt_name} ===")
        acc_map = aggregate_results(directory=ckpt_dir)  # <- 这里拿到返回值
        update_best(acc_map, ckpt_name, ckpt_dir)

    # 输出最大值
    if best["checkpoint"] is None:
        print(f"No results found under: {root_dir}")
    else:
        print("\n=== BEST ===")
        print(f"checkpoint: {best['checkpoint']}")
        print(f"setup     : {best['setup']}")
        print(f"accuracy  : {best['accuracy']:.2f}%")
        print(f"path      : {best['dir']}")
    ...
    for item in data.get("generations", []):
        ...
        if not parsed_answer:
            ...
            if answer_match:
                ...
                # Now extract from \box{...}
                box_match = re.search(r"\\box\{(.+?)\}", parsed_answer)
                if box_match:
                    parsed_answer = box_match.group(1).strip() 
        is_correct = False
...
def aggregate_results(directory="."):
    ...
    for setup, results in sorted(setups.items()):
        ...
        acc_map[setup] = results["accuracy"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--directory', type=str, default='eval_results',
                        help='eval_results directory containing checkpoint-* subdirs (or json files)')
    args = parser.parse_args()

    aggregate_all_checkpoints(args.directory)