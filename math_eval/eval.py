...
# from new_generate import generate
...
def evaluate(
    ...
    for batch in tqdm(dataloader, disable=(dist.get_rank() != 0)):
        ...
        if "LLaDA" in model.config.name_or_path:
            out = generate(
                model,
                input_ids,
                tokenizer,
...
if __name__ == "__main__":
    ...
    # Use environment variable for cache directory, fallback to default Hugging Face cache
    ...
    if args.checkpoint_path:
        # Hugging Face repository - use from_pretrained
    ...
    print("name_or_path:", getattr(model.config, "name_or_path", None))
