...
# Official DiffusionGemma thinking-mode math RL on BytedTsinghua-SIA/DAPO-Math-17k.
# Dataset loader: DiffGemma.diffgemma_trl.data_utils.get_dapo_math_17k_questions
...
# ---- Training config ----
# 2048-token rollouts are expensive; keep per-device batch small.
PER_DEVICE_TRAIN_BATCH_SIZE=1
GRADIENT_ACCUMULATION_STEPS=2
NUM_GENERATIONS=8
# Constraints: GENERATION_BATCH_SIZE >= batch * world * grad_accum, and divisible by NUM_GENERATIONS.
RAW_GENERATION_BATCH_SIZE=$((PER_DEVICE_TRAIN_BATCH_SIZE * WORLD_SIZE * GRADIENT_ACCUMULATION_STEPS))
GENERATION_BATCH_SIZE=$(( ((RAW_GENERATION_BATCH_SIZE + NUM_GENERATIONS - 1) / NUM_GENERATIONS) * NUM_GENERATIONS ))
NUM_ITERATIONS=1
LEARNING_RATE=1e-6
MAX_PROMPT_LENGTH=1024
MAX_COMPLETION_LENGTH=2048
DIFFUSION_CANVAS_LENGTH=256
# max_completion_length (2048) > canvas_length (256): score the LAST canvas_length
# tokens of each completion so the RL loss window includes the final answer, instead
# of the first 256 tokens (often shared reasoning setup, decoupled from the reward).
CANVAS_WINDOW="end"
MAX_STEPS=2000
...
# Output
RUN_NAME="${DATASET}_think2048_diffgemma_${RL_LOSS_TYPE}_tielora_r${LORA_R}_alpha${LORA_ALPHA}_mc${DIFFUSION_NUM_MC}_lr${LEARNING_RATE}_seed${SEED}"
